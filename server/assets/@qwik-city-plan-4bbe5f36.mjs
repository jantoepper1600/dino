...
const processData$1 = (node, invocationContext) => {
  if (null != node && "boolean" != typeof node) {
    ...
    return isPromise(node) ? node.then((node2) => processData$1(node2, invocationContext)) : node === SkipRender ? new ProcessedJSXNodeImpl(SKIP_RENDER_TYPE, EMPTY_OBJ, null, EMPTY_ARRAY, 0, null) : void logWarn();
  }
};
...
const resumeContainer = (containerEl) => {
  ...
  const computeObject = (id) => {
    if (id.startsWith("#")) {
      ...
      assertTrue(elements.has(index3));
...
const executeTasksBefore = async (containerState, rCtx) => {
  ...
  const isResourceTask2 = (task) => 0 != (task.$flags$ & TaskFlagsIsResource);
  containerState.$taskNext$.forEach((task) => {
    isTask(task) && (taskPromises.push(maybeThen(task.$qrl$.$resolveLazy$(containerEl), () => task)), containerState.$taskNext$.delete(task)), isResourceTask2(task) && (resourcesPromises.push(maybeThen(task.$qrl$.$resolveLazy$(containerEl), () => task)), containerState.$taskNext$.delete(task));
  });
  do {
    if (containerState.$taskStaging$.forEach((task) => {
      isTask(task) ? taskPromises.push(maybeThen(task.$qrl$.$resolveLazy$(containerEl), () => task)) : isResourceTask2(task) ? resourcesPromises.push(maybeThen(task.$qrl$.$resolveLazy$(containerEl), () => task)) : containerState.$taskNext$.add(task);
...
const useVisibleTaskQrl = (qrl, opts) => {
  ...
  const containerState = iCtx.$renderCtx$.$static$.$containerState$;
  elCtx.$tasks$ || (elCtx.$tasks$ = []), elCtx.$tasks$.push(task), set(task), useRunTask(task, eagerness), isServerPlatform() || (qrl.$resolveLazy$(containerState.$containerEl$), notifyTask(task, containerState));
};
...
const runTask = (task, containerState, rCtx) => {
  ...
  const opts = {
    track: (obj, prop) => {
      ...
      const manager = getSubscriptionManager(obj);
      return manager ? manager.$addSub$([0, task], prop) : logErrorAndStop(codeToText(26), obj), prop ? obj[prop] : obj;
    },
    cleanup(callback) {
      cleanups.push(callback);
    }
...
const _renderSSR = async (node, opts) => {
  ...
  "html" !== root && (containerAttributes.class = "qc📦" + (containerAttributes.class ? " " + containerAttributes.class : "")), opts.serverData && (containerState.$serverData$ = opts.serverData), node = _jsxQ(root, null, containerAttributes, children, HOST_FLAG_DIRTY | HOST_FLAG_NEED_ATTACH_LISTENER, null), containerState.$hostsRendering$ = /* @__PURE__ */ new Set(), await Promise.resolve().then(() => renderRoot$1(node, rCtx, ssrCtx, opts.stream, containerState, opts));
};
const renderRoot$1 = async (node, rCtx, ssrCtx, stream, containerState, opts) => {
  ...
  return await renderNode(node, rCtx, ssrCtx, stream, 0, beforeClose ? (stream2) => {
    ...
    return processData(result, rCtx, ssrCtx, stream2, 0, void 0);
  } : void 0), rCtx;
};
  ...
  return maybeThen(promise, () => {
    ...
    if (isSlot) {
      ...
      if (content) {
        ...
        newSlotRctx.$slotCtx$ = elCtx, ssrCtx.$projectedChildren$[key] = void 0, promise2 = processData(content, newSlotRctx, sCtx, stream, flags);
      }
    }
...
const renderSSRComponent = (rCtx, ssrCtx, stream, elCtx, node, flags, beforeClose) => (setComponentProps$1(rCtx, elCtx, node.props.props), maybeThen(executeComponent(rCtx, elCtx), (res) => {
  ...
  return elCtx.$id$ = newID, ssrCtx.$static$.$contexts$.push(elCtx), renderNodeVirtual(processedNode, elCtx, extraNodes, newRCtx, newSSrContext, stream, flags, (stream2) => {
    ...
    if (projectedChildren) {
      ...
      newSlotRctx.$slotCtx$ = elCtx, missingSlotsDone = processData(nodes, newSlotRctx, sCtx, stream2, 0, void 0);
    }
...
const renderNode = (node, rCtx, ssrCtx, stream, flags, beforeClose) => {
  ...
  if ("string" == typeof tagName) {
    ...
    if (immutable) {
      for (const prop in immutable) {
        ...
        if (isSignal(value) && (value = trackSignal(value, [1, elm, value, hostCtx.$element$, attrName]), useSignal2 = true), prop === dangerouslySetInnerHTML) {
          htmlStr = value;
          continue;
        }
    ...
    for (const prop in props) {
      ...
      const attrName = processPropKey(prop);
      if (isSignal(value) && (value = trackSignal(value, [2, hostCtx.$element$, value, elm, attrName]), useSignal2 = true), prop === dangerouslySetInnerHTML) {
        htmlStr = value;
        continue;
      }
      prop.startsWith(PREVENT_DEFAULT) && registerQwikEvent$1(prop.slice(15), rCtx.$static$.$containerState$);
  ...
  if (tagName === Virtual) {
    ...
    return elCtx.$parentCtx$ = rCtx.$slotCtx$ || rCtx.$cmpCtx$, hostCtx && 8 & hostCtx.$flags$ && addDynamicSlot(hostCtx, elCtx), renderNodeVirtual(node, elCtx, void 0, rCtx, ssrCtx, stream, flags, beforeClose);
  }
  if (tagName === SSRRaw) {
...
const processData = (node, rCtx, ssrCtx, stream, flags, beforeClose) => {
  ...
  if (null != node && "boolean" != typeof node) {
    if (!isString(node) && "number" != typeof node) {
      ...
      return isPromise(node) ? (stream.write(FLUSH_COMMENT), node.then((node2) => processData(node2, rCtx, ssrCtx, stream, flags, beforeClose))) : void logWarn();
    }
...
const checkBeforeAssign = (ctx, elm, newValue, prop) => (prop in elm && elm[prop] !== newValue && ("SELECT" === elm.tagName ? setPropertyPost(ctx, elm, prop, newValue) : setProperty(ctx, elm, prop, newValue)), true);
...
const DerivedSignalSerializer = /* @__PURE__ */ serializer({
  ...
  $serialize$: (signal, getObjID, collector) => {
    ...
    return index2 < 0 && (collector.$inlinedFunctions$.push(serialized), index2 = collector.$inlinedFunctions$.length - 1), mapJoin(signal.$args$, getObjID, " ") + " @" + intToStr(index2);
  },
  $prepare$: (data) => {
...
const s_B0lqk5IDDy4 = () => {
  ...
  return /* @__PURE__ */ _jsxQ("main", null, {
    class: _fnSignal((p0) => [
      ...
      state
    ], '["bg-blue",p0.value==1&&"bg-red"]')
  }, [
    /* @__PURE__ */ _jsxQ("div", null, {
      id: "ovrly",
      onClick$: removeActive
    }, null, 3, null),
    /* @__PURE__ */ _jsxQ("nav", null, null, [
    ...
    /* @__PURE__ */ _jsxQ("div", null, {
      ...
      /* @__PURE__ */ _jsxQ("div", null, {
        ...
        /* @__PURE__ */ _jsxQ("div", null, {
          ...
          /* @__PURE__ */ _jsxQ("div", null, {
            ...
            /* @__PURE__ */ _jsxQ("div", null, {
              ...
              /* @__PURE__ */ _jsxQ("h3", null, null, "Verhältnis der Zeitalter", 3, null),
              /* @__PURE__ */ _jsxQ("p", null, null, "Legende:", 3, null),
              /* @__PURE__ */ _jsxQ("ul", null, null, [
                /* @__PURE__ */ _jsxQ("li", null, null, "Zeitalter der Dinosaurier", 3, null),
                /* @__PURE__ */ _jsxQ("li", null, null, "Auslöschung", 3, null),
                /* @__PURE__ */ _jsxQ("li", null, null, "Heute", 3, null)
              ], 3, null)
            ], 3, null),
            /* @__PURE__ */ _jsxQ("div", null, {
              class: "chart"
          ...
          /* @__PURE__ */ _jsxQ("p", null, null, [
            "Das Zeitalter der Dinsaurier begann vor ",
            /* @__PURE__ */ _jsxQ("strong", null, null, "245 Millionen Jahren", 3, null),
            " und hielt ",
            /* @__PURE__ */ _jsxQ("strong", null, null, "180 Millionen Jahre", 3, null),
            " an.",
        ...
        /* @__PURE__ */ _jsxQ("div", null, {
          ...
          /* @__PURE__ */ _jsxQ("p", null, null, [
            "Vor ",
            /* @__PURE__ */ _jsxQ("strong", null, null, "65 Millionen Jahren", 3, null),
            " schlug ein Asteroid auf die Erde ein und löste eine Kettenreaktion von Vulkanausbrüchen aus, die den Himmel mit schwarzen Wolken bedeckten.",
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            "Das hatte zur Folge, dass das Sonnenlicht von der Wolckendecke blockiert wurde und in Kombination mit den Vulkanausbrüchen das globale Klima drastisch veränderte.",
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            "Es starben nahezu alle Dinosaurier aus und man spircht von der Auslöschung, bzw dem Massensterben der Dinosaurier."
          ], 3, null)
        ], 3, null),
        /* @__PURE__ */ _jsxQ("div", null, {
          class: "dino",
          ...
          /* @__PURE__ */ _jsxQ("p", null, null, [
            "Wir Menschen, die Homo sapiens, existieren seit ",
            /* @__PURE__ */ _jsxQ("strong", null, null, "ca. 300.000 Jahren", 3, null),
            ".",
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            "Damit haben wir im Vergleich mit Abstand das jüngste und kürzeste Zeitalter.",
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            "Trotzdem haben wir es geschafft uns in kurzer Zeit exponentiell zu einer hohen und komplexen Lebensform weiterzuentwickeln."
          ], 3, null)
        ], 3, null),
        /* @__PURE__ */ _jsxQ("div", null, {
          class: "spezies",
          ...
          /* @__PURE__ */ _jsxQ("div", null, {
            ...
            /* @__PURE__ */ _jsxQ("div", null, {
              ...
              /* @__PURE__ */ _jsxQ("h3", null, null, "Verhältnis der Spezienkategorien", 3, null),
              /* @__PURE__ */ _jsxQ("p", null, null, "Legende", 3, null),
              /* @__PURE__ */ _jsxQ("ul", null, null, [
                /* @__PURE__ */ _jsxQ("li", null, null, "Entdeckte Dinosaurierarten", 3, null),
                /* @__PURE__ */ _jsxQ("li", null, null, "Unentdeckte Arten", 3, null),
                /* @__PURE__ */ _jsxQ("li", null, null, "Entdeckte Tierarten", 3, null)
              ], 3, null)
            ], 3, null),
            /* @__PURE__ */ _jsxQ("div", null, {
              class: "bar"
            }, [
              /* @__PURE__ */ _jsxQ("hr", null, null, null, 3, null),
              /* @__PURE__ */ _jsxQ("hr", null, null, null, 3, null),
              /* @__PURE__ */ _jsxQ("hr", null, null, null, 3, null)
            ], 3, null)
          ], 3, null),
          /* @__PURE__ */ _jsxQ("p", null, null, [
            "Es wurden bislang nur ",
            /* @__PURE__ */ _jsxQ("strong", null, null, "700 verschiedene Dinosaurierspezien", 3, null),
            " entdeckt und es wird davon ausgegangen, dass es noch weit mehr Dinosaurierspezien gibt.",
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            "Das liegt daran, dass Fossilien schwer zu entdecken sind und tief unter der Erde liegen."
          ], 3, null)
        ], 3, null),
        /* @__PURE__ */ _jsxQ("div", null, {
          class: "spezies",
          ...
          /* @__PURE__ */ _jsxQ("p", null, null, [
            "Experten schätzen, dass es insgesamt ",
            /* @__PURE__ */ _jsxQ("strong", null, null, "15 Millionen verschiedenen Arten", 3, null),
            " in den ",
            /* @__PURE__ */ _jsxQ("strong", null, null, "245 Millionen Jahren", 3, null),
            " existiert haben.",
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            "Davon sind bis heute fast alle unentdeckt und es werden täglich neue Arten entdeckt."
          ], 3, null)
        ], 3, null),
        /* @__PURE__ */ _jsxQ("div", null, {
          class: "spezies",
          ...
          /* @__PURE__ */ _jsxQ("p", null, null, [
            "Unter unserer heutigen Tierwelt gibt es um die ",
            /* @__PURE__ */ _jsxQ("strong", null, null, "1,8 Millionen verschiedene entdeckte Arten", 3, null),
            ".",
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            "Davon sind die meisten Tiere auf dem Land entdeckt worden und die wenigsten im Meer.",
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            "Hier gilt bei den Meerestieren der selbe Grund, wie bei der Entdeckung der Dinosaurier.",
            /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
            "Es ist für uns Menschen schwer neue Meerestiere zu entdecken, durch die erschwerten Bedienungen ab einer bestimmten Meerestiefe."
          ], 3, null)
        ], 3, null),
        /* @__PURE__ */ _jsxQ("div", null, {
          class: "flora",
          ...
          "Zu der Flora im Dinosaurierzeitalter ist leider wenig bekannt.",
          /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
          "Das liegt daran, dass Pflanzen im Gegensatz zu den Dinosaurier keine Knochen besitzen, die durch die Versteinerung für uns Menschen erhalten bleiben. Es gibt nur wenige Funde, doch man konnte feststellen, dass die damalige Flora unserer heutigen Flora recht ähnlich war.",
          /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
          "So wuchsen damals schon verschiendene Baumfarnarten in den Wäldern der Dinosaurier."
        ], 3, null), 3, null),
        /* @__PURE__ */ _jsxQ("div", null, {
          class: "flora",
          id: "phase2-flora"
        }, /* @__PURE__ */ _jsxQ("p", null, null, [
          "Durch die globale Verwüstungung des Asteroiden, starb sowohl die Flora und Fauna.",
          /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
          "Es gab kein Sonnenlicht und die Erde kühlte nach mit der Zeit immer weiter ab.",
          /* @__PURE__ */ _jsxQ("br", null, null, null, 3, null),
          "Es dauerte um die ",
          /* @__PURE__ */ _jsxQ("strong", null, null, "25 Millionen Jahre", 3, null),
          " bis neue Ökosysteme entstehen konnten."
        ], 3, null), 3, null),
        /* @__PURE__ */ _jsxQ("div", null, {
          class: "flora",
          id: "phase3-flora"
